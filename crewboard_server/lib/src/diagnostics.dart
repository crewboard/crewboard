...
void main(List<String> args) async {
  ...
  try {
    stdout.writeln('Checking custom User table...');
    final users = await User.db.find(
      session,
      where: (t) => t.userName.equals('jerin'),
    );
    if (users.isEmpty) {
      stdout.writeln('No user "jerin" found in custom users table.');
    } else {
    ...
    if (profiles.isEmpty) {
      stdout.writeln('No UserProfile found for "jerin".');
    } else {
      for (var profile in profiles) {
        stdout.writeln(
          'Found UserProfile: id=${profile.id}, userName=${profile.userName}, email=${profile.email}',
        );
      }
    }
    ...
    if (result.isEmpty) {
      stdout.writeln('No EmailAuth record found for email containing "jerin".');
    } else {
      for (var row in result) {
        stdout.writeln('Found EmailAuth: $row');
      }
    }
  } catch (e) {
    stdout.writeln('Error during diagnostics: $e');
  } finally {
    await session.close();
  }
}
