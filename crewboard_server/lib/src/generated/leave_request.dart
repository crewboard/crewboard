/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: unnecessary_null_comparison

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:serverpod/serverpod.dart' as _i1;
import 'user.dart' as _i2;
import 'package:crewboard_server/src/generated/protocol.dart' as _i3;

abstract class LeaveRequest
    implements _i1.TableRow<_i1.UuidValue?>, _i1.ProtocolSerialization {
  LeaveRequest._({
    this.id,
    required this.userId,
    this.user,
    required this.request,
    this.accepted,
    this.date,
  });

  factory LeaveRequest({
    _i1.UuidValue? id,
    required _i1.UuidValue userId,
    _i2.User? user,
    required String request,
    bool? accepted,
    DateTime? date,
  }) = _LeaveRequestImpl;

  factory LeaveRequest.fromJson(Map<String, dynamic> jsonSerialization) {
    return LeaveRequest(
      id: jsonSerialization['id'] == null
          ? null
          : _i1.UuidValueJsonExtension.fromJson(jsonSerialization['id']),
      userId: _i1.UuidValueJsonExtension.fromJson(jsonSerialization['userId']),
      user: jsonSerialization['user'] == null
          ? null
          : _i3.Protocol().deserialize<_i2.User>(jsonSerialization['user']),
      request: jsonSerialization['request'] as String,
      accepted: jsonSerialization['accepted'] as bool?,
      date: jsonSerialization['date'] == null
          ? null
          : _i1.DateTimeJsonExtension.fromJson(jsonSerialization['date']),
    );
  }

  static final t = LeaveRequestTable();

  static const db = LeaveRequestRepository._();

  @override
  _i1.UuidValue? id;

  _i1.UuidValue userId;

  _i2.User? user;

  String request;

  bool? accepted;

  DateTime? date;

  @override
  _i1.Table<_i1.UuidValue?> get table => t;

  /// Returns a shallow copy of this [LeaveRequest]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  LeaveRequest copyWith({
    _i1.UuidValue? id,
    _i1.UuidValue? userId,
    _i2.User? user,
    String? request,
    bool? accepted,
    DateTime? date,
  });
  @override
  Map<String, dynamic> toJson() {
    return {
      '__className__': 'LeaveRequest',
      if (id != null) 'id': id?.toJson(),
      'userId': userId.toJson(),
      if (user != null) 'user': user?.toJson(),
      'request': request,
      if (accepted != null) 'accepted': accepted,
      if (date != null) 'date': date?.toJson(),
    };
  }

  @override
  Map<String, dynamic> toJsonForProtocol() {
    return {
      '__className__': 'LeaveRequest',
      if (id != null) 'id': id?.toJson(),
      'userId': userId.toJson(),
      if (user != null) 'user': user?.toJsonForProtocol(),
      'request': request,
      if (accepted != null) 'accepted': accepted,
      if (date != null) 'date': date?.toJson(),
    };
  }

  static LeaveRequestInclude include({_i2.UserInclude? user}) {
    return LeaveRequestInclude._(user: user);
  }

  static LeaveRequestIncludeList includeList({
    _i1.WhereExpressionBuilder<LeaveRequestTable>? where,
    int? limit,
    int? offset,
    _i1.OrderByBuilder<LeaveRequestTable>? orderBy,
    bool orderDescending = false,
    _i1.OrderByListBuilder<LeaveRequestTable>? orderByList,
    LeaveRequestInclude? include,
  }) {
    return LeaveRequestIncludeList._(
      where: where,
      limit: limit,
      offset: offset,
      orderBy: orderBy?.call(LeaveRequest.t),
      orderDescending: orderDescending,
      orderByList: orderByList?.call(LeaveRequest.t),
      include: include,
    );
  }

  @override
  String toString() {
    return _i1.SerializationManager.encode(this);
  }
}

class _Undefined {}

class _LeaveRequestImpl extends LeaveRequest {
  _LeaveRequestImpl({
    _i1.UuidValue? id,
    required _i1.UuidValue userId,
    _i2.User? user,
    required String request,
    bool? accepted,
    DateTime? date,
  }) : super._(
         id: id,
         userId: userId,
         user: user,
         request: request,
         accepted: accepted,
         date: date,
       );

  /// Returns a shallow copy of this [LeaveRequest]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  @override
  LeaveRequest copyWith({
    Object? id = _Undefined,
    _i1.UuidValue? userId,
    Object? user = _Undefined,
    String? request,
    Object? accepted = _Undefined,
    Object? date = _Undefined,
  }) {
    return LeaveRequest(
      id: id is _i1.UuidValue? ? id : this.id,
      userId: userId ?? this.userId,
      user: user is _i2.User? ? user : this.user?.copyWith(),
      request: request ?? this.request,
      accepted: accepted is bool? ? accepted : this.accepted,
      date: date is DateTime? ? date : this.date,
    );
  }
}

class LeaveRequestUpdateTable extends _i1.UpdateTable<LeaveRequestTable> {
  LeaveRequestUpdateTable(super.table);

  _i1.ColumnValue<_i1.UuidValue, _i1.UuidValue> userId(_i1.UuidValue value) =>
      _i1.ColumnValue(
        table.userId,
        value,
      );

  _i1.ColumnValue<String, String> request(String value) => _i1.ColumnValue(
    table.request,
    value,
  );

  _i1.ColumnValue<bool, bool> accepted(bool? value) => _i1.ColumnValue(
    table.accepted,
    value,
  );

  _i1.ColumnValue<DateTime, DateTime> date(DateTime? value) => _i1.ColumnValue(
    table.date,
    value,
  );
}

class LeaveRequestTable extends _i1.Table<_i1.UuidValue?> {
  LeaveRequestTable({super.tableRelation}) : super(tableName: 'leave_request') {
    updateTable = LeaveRequestUpdateTable(this);
    userId = _i1.ColumnUuid(
      'userId',
      this,
    );
    request = _i1.ColumnString(
      'request',
      this,
    );
    accepted = _i1.ColumnBool(
      'accepted',
      this,
    );
    date = _i1.ColumnDateTime(
      'date',
      this,
    );
  }

  late final LeaveRequestUpdateTable updateTable;

  late final _i1.ColumnUuid userId;

  _i2.UserTable? _user;

  late final _i1.ColumnString request;

  late final _i1.ColumnBool accepted;

  late final _i1.ColumnDateTime date;

  _i2.UserTable get user {
    if (_user != null) return _user!;
    _user = _i1.createRelationTable(
      relationFieldName: 'user',
      field: LeaveRequest.t.userId,
      foreignField: _i2.User.t.id,
      tableRelation: tableRelation,
      createTable: (foreignTableRelation) =>
          _i2.UserTable(tableRelation: foreignTableRelation),
    );
    return _user!;
  }

  @override
  List<_i1.Column> get columns => [
    id,
    userId,
    request,
    accepted,
    date,
  ];

  @override
  _i1.Table? getRelationTable(String relationField) {
    if (relationField == 'user') {
      return user;
    }
    return null;
  }
}

class LeaveRequestInclude extends _i1.IncludeObject {
  LeaveRequestInclude._({_i2.UserInclude? user}) {
    _user = user;
  }

  _i2.UserInclude? _user;

  @override
  Map<String, _i1.Include?> get includes => {'user': _user};

  @override
  _i1.Table<_i1.UuidValue?> get table => LeaveRequest.t;
}

class LeaveRequestIncludeList extends _i1.IncludeList {
  LeaveRequestIncludeList._({
    _i1.WhereExpressionBuilder<LeaveRequestTable>? where,
    super.limit,
    super.offset,
    super.orderBy,
    super.orderDescending,
    super.orderByList,
    super.include,
  }) {
    super.where = where?.call(LeaveRequest.t);
  }

  @override
  Map<String, _i1.Include?> get includes => include?.includes ?? {};

  @override
  _i1.Table<_i1.UuidValue?> get table => LeaveRequest.t;
}

class LeaveRequestRepository {
  const LeaveRequestRepository._();

  final attachRow = const LeaveRequestAttachRowRepository._();

  /// Returns a list of [LeaveRequest]s matching the given query parameters.
  ///
  /// Use [where] to specify which items to include in the return value.
  /// If none is specified, all items will be returned.
  ///
  /// To specify the order of the items use [orderBy] or [orderByList]
  /// when sorting by multiple columns.
  ///
  /// The maximum number of items can be set by [limit]. If no limit is set,
  /// all items matching the query will be returned.
  ///
  /// [offset] defines how many items to skip, after which [limit] (or all)
  /// items are read from the database.
  ///
  /// ```dart
  /// var persons = await Persons.db.find(
  ///   session,
  ///   where: (t) => t.lastName.equals('Jones'),
  ///   orderBy: (t) => t.firstName,
  ///   limit: 100,
  /// );
  /// ```
  Future<List<LeaveRequest>> find(
    _i1.Session session, {
    _i1.WhereExpressionBuilder<LeaveRequestTable>? where,
    int? limit,
    int? offset,
    _i1.OrderByBuilder<LeaveRequestTable>? orderBy,
    bool orderDescending = false,
    _i1.OrderByListBuilder<LeaveRequestTable>? orderByList,
    _i1.Transaction? transaction,
    LeaveRequestInclude? include,
  }) async {
    return session.db.find<LeaveRequest>(
      where: where?.call(LeaveRequest.t),
      orderBy: orderBy?.call(LeaveRequest.t),
      orderByList: orderByList?.call(LeaveRequest.t),
      orderDescending: orderDescending,
      limit: limit,
      offset: offset,
      transaction: transaction,
      include: include,
    );
  }

  /// Returns the first matching [LeaveRequest] matching the given query parameters.
  ///
  /// Use [where] to specify which items to include in the return value.
  /// If none is specified, all items will be returned.
  ///
  /// To specify the order use [orderBy] or [orderByList]
  /// when sorting by multiple columns.
  ///
  /// [offset] defines how many items to skip, after which the next one will be picked.
  ///
  /// ```dart
  /// var youngestPerson = await Persons.db.findFirstRow(
  ///   session,
  ///   where: (t) => t.lastName.equals('Jones'),
  ///   orderBy: (t) => t.age,
  /// );
  /// ```
  Future<LeaveRequest?> findFirstRow(
    _i1.Session session, {
    _i1.WhereExpressionBuilder<LeaveRequestTable>? where,
    int? offset,
    _i1.OrderByBuilder<LeaveRequestTable>? orderBy,
    bool orderDescending = false,
    _i1.OrderByListBuilder<LeaveRequestTable>? orderByList,
    _i1.Transaction? transaction,
    LeaveRequestInclude? include,
  }) async {
    return session.db.findFirstRow<LeaveRequest>(
      where: where?.call(LeaveRequest.t),
      orderBy: orderBy?.call(LeaveRequest.t),
      orderByList: orderByList?.call(LeaveRequest.t),
      orderDescending: orderDescending,
      offset: offset,
      transaction: transaction,
      include: include,
    );
  }

  /// Finds a single [LeaveRequest] by its [id] or null if no such row exists.
  Future<LeaveRequest?> findById(
    _i1.Session session,
    _i1.UuidValue id, {
    _i1.Transaction? transaction,
    LeaveRequestInclude? include,
  }) async {
    return session.db.findById<LeaveRequest>(
      id,
      transaction: transaction,
      include: include,
    );
  }

  /// Inserts all [LeaveRequest]s in the list and returns the inserted rows.
  ///
  /// The returned [LeaveRequest]s will have their `id` fields set.
  ///
  /// This is an atomic operation, meaning that if one of the rows fails to
  /// insert, none of the rows will be inserted.
  Future<List<LeaveRequest>> insert(
    _i1.Session session,
    List<LeaveRequest> rows, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.insert<LeaveRequest>(
      rows,
      transaction: transaction,
    );
  }

  /// Inserts a single [LeaveRequest] and returns the inserted row.
  ///
  /// The returned [LeaveRequest] will have its `id` field set.
  Future<LeaveRequest> insertRow(
    _i1.Session session,
    LeaveRequest row, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.insertRow<LeaveRequest>(
      row,
      transaction: transaction,
    );
  }

  /// Updates all [LeaveRequest]s in the list and returns the updated rows. If
  /// [columns] is provided, only those columns will be updated. Defaults to
  /// all columns.
  /// This is an atomic operation, meaning that if one of the rows fails to
  /// update, none of the rows will be updated.
  Future<List<LeaveRequest>> update(
    _i1.Session session,
    List<LeaveRequest> rows, {
    _i1.ColumnSelections<LeaveRequestTable>? columns,
    _i1.Transaction? transaction,
  }) async {
    return session.db.update<LeaveRequest>(
      rows,
      columns: columns?.call(LeaveRequest.t),
      transaction: transaction,
    );
  }

  /// Updates a single [LeaveRequest]. The row needs to have its id set.
  /// Optionally, a list of [columns] can be provided to only update those
  /// columns. Defaults to all columns.
  Future<LeaveRequest> updateRow(
    _i1.Session session,
    LeaveRequest row, {
    _i1.ColumnSelections<LeaveRequestTable>? columns,
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateRow<LeaveRequest>(
      row,
      columns: columns?.call(LeaveRequest.t),
      transaction: transaction,
    );
  }

  /// Updates a single [LeaveRequest] by its [id] with the specified [columnValues].
  /// Returns the updated row or null if no row with the given id exists.
  Future<LeaveRequest?> updateById(
    _i1.Session session,
    _i1.UuidValue id, {
    required _i1.ColumnValueListBuilder<LeaveRequestUpdateTable> columnValues,
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<LeaveRequest>(
      id,
      columnValues: columnValues(LeaveRequest.t.updateTable),
      transaction: transaction,
    );
  }

  /// Updates all [LeaveRequest]s matching the [where] expression with the specified [columnValues].
  /// Returns the list of updated rows.
  Future<List<LeaveRequest>> updateWhere(
    _i1.Session session, {
    required _i1.ColumnValueListBuilder<LeaveRequestUpdateTable> columnValues,
    required _i1.WhereExpressionBuilder<LeaveRequestTable> where,
    int? limit,
    int? offset,
    _i1.OrderByBuilder<LeaveRequestTable>? orderBy,
    _i1.OrderByListBuilder<LeaveRequestTable>? orderByList,
    bool orderDescending = false,
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateWhere<LeaveRequest>(
      columnValues: columnValues(LeaveRequest.t.updateTable),
      where: where(LeaveRequest.t),
      limit: limit,
      offset: offset,
      orderBy: orderBy?.call(LeaveRequest.t),
      orderByList: orderByList?.call(LeaveRequest.t),
      orderDescending: orderDescending,
      transaction: transaction,
    );
  }

  /// Deletes all [LeaveRequest]s in the list and returns the deleted rows.
  /// This is an atomic operation, meaning that if one of the rows fail to
  /// be deleted, none of the rows will be deleted.
  Future<List<LeaveRequest>> delete(
    _i1.Session session,
    List<LeaveRequest> rows, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.delete<LeaveRequest>(
      rows,
      transaction: transaction,
    );
  }

  /// Deletes a single [LeaveRequest].
  Future<LeaveRequest> deleteRow(
    _i1.Session session,
    LeaveRequest row, {
    _i1.Transaction? transaction,
  }) async {
    return session.db.deleteRow<LeaveRequest>(
      row,
      transaction: transaction,
    );
  }

  /// Deletes all rows matching the [where] expression.
  Future<List<LeaveRequest>> deleteWhere(
    _i1.Session session, {
    required _i1.WhereExpressionBuilder<LeaveRequestTable> where,
    _i1.Transaction? transaction,
  }) async {
    return session.db.deleteWhere<LeaveRequest>(
      where: where(LeaveRequest.t),
      transaction: transaction,
    );
  }

  /// Counts the number of rows matching the [where] expression. If omitted,
  /// will return the count of all rows in the table.
  Future<int> count(
    _i1.Session session, {
    _i1.WhereExpressionBuilder<LeaveRequestTable>? where,
    int? limit,
    _i1.Transaction? transaction,
  }) async {
    return session.db.count<LeaveRequest>(
      where: where?.call(LeaveRequest.t),
      limit: limit,
      transaction: transaction,
    );
  }
}

class LeaveRequestAttachRowRepository {
  const LeaveRequestAttachRowRepository._();

  /// Creates a relation between the given [LeaveRequest] and [User]
  /// by setting the [LeaveRequest]'s foreign key `userId` to refer to the [User].
  Future<void> user(
    _i1.Session session,
    LeaveRequest leaveRequest,
    _i2.User user, {
    _i1.Transaction? transaction,
  }) async {
    if (leaveRequest.id == null) {
      throw ArgumentError.notNull('leaveRequest.id');
    }
    if (user.id == null) {
      throw ArgumentError.notNull('user.id');
    }

    var $leaveRequest = leaveRequest.copyWith(userId: user.id);
    await session.db.updateRow<LeaveRequest>(
      $leaveRequest,
      columns: [LeaveRequest.t.userId],
      transaction: transaction,
    );
  }
}
