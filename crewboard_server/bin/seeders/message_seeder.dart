...
void main(List<String> args) async {
  ...
  try {
    ...
    for (int i = 0; i < shuffledUsers.length; i += 2) {
      ...
      final user2 = shuffledUsers[(i + 1) % shuffledUsers.length]; // Wrap around if odd number
    ...
    if (seedfilesDir.existsSync()) {
      for (var type in fileCache.keys) {
        ...
        if (dir.existsSync()) {
           ...
           if (!targetSubDir.existsSync()) targetSubDir.createSync(recursive: true);
        }
      }
    } else {
      print('Warning: bin/seeders/files directory not found. File messages will be skipped.');
    }
    ...
    for (final entry in roomsWithUsers.entries) {
       ...
       for (var i = 0; i < messageCount; i++) {
         ...
         conversationTime = conversationTime.add(Duration(minutes: 2 + random.nextInt(58)));
         ...
         if (burstRemaining <= 0) {
            ...
            burstRemaining = random.nextInt(3) + 1; // Start new burst of 1-3 messages
         }
         ...
         if (i % 10 == 0 && fileCache['images']!.isNotEmpty) {
           ...
           await _sendFileMessage(session, room, sender, fileCache['images']!, 'images', MessageType.image, createdAt, random);
         } else if (i % 15 == 0 && fileCache['videos']!.isNotEmpty) {
           // Video message every 15th message
           await _sendFileMessage(session, room, sender, fileCache['videos']!, 'videos', MessageType.video, createdAt, random);
         } else if (i % 20 == 0 && fileCache['audios']!.isNotEmpty) {
           // Audio message every 20th message
           await _sendFileMessage(session, room, sender, fileCache['audios']!, 'audios', MessageType.audio, createdAt, random);
         } else if (i % 25 == 0 && fileCache['docs']!.isNotEmpty) {
           // Document message every 25th message
           await _sendFileMessage(session, room, sender, fileCache['docs']!, 'docs', MessageType.file, createdAt, random);
         } else {
           // Text message
       ...
       print('   └─ ${user1.userName}: $user1MessageCount messages, ${user2.userName}: $user2MessageCount messages');
    }
