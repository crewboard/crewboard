...
class DocGenerator {
  ...
  String generateDeltaJson(String docTitle) {
    ...
    ops.add({
      "insert":
          "This document outlines the detailed specifications for $docTitle. "
    ...
    } else if (docTitle.contains("Architecture") ||
        docTitle.contains("Infrastructure") ||
        docTitle.contains("Database")) {
      _addTechnicalDeepDive(ops);
    } else if (docTitle.contains("API") ||
        docTitle.contains("Frontend") ||
        docTitle.contains("Security")) {
  ...
  void _addDetailedRequirements(List<Map<String, dynamic>> ops) {
    ...
    ops.add({
      "insert": "1.1 In-Scope Items\n",
      "attributes": {"header": 3},
    });
    ops.add({
      "insert":
          "• Core feature implementation including authentication and user management.\n",
    });
    ...
    ops.add({
      "insert": "• Legacy data migration (handled in a separate phase).\n",
    });
  ...
  void _addTechnicalDeepDive(List<Map<String, dynamic>> ops) {
    ...
    ops.add({
      "insert":
          "The system utilizes a Hexagonal Architecture pattern to isolate core business logic from external concerns.\n",
    });
    ...
    ops.add({
      "insert": ": Handles routing, rate limiting, and authentication.\n",
    });
    ops.add({"insert": "2. "});
    ops.add({
      "insert": "Service Mesh",
    ...
    ops.add({
      "insert": ": PostgreSQL for relational data, Redis for caching.\n",
    });
    ...
    ops.add({
      "insert":
          "All endpoints must respond within 200ms at 95th percentile. Database queries should be optimized using appropriate indexing strategies.\n",
    });
  }
  ...
  void _addSpecificationDetails(List<Map<String, dynamic>> ops) {
    ...
    ops.add({
      "insert":
          "All data in transit is encrypted via TLS 1.3. At rest, sensitive fields are hashed using Argon2id.\n",
    });
  }
  ...
  void _addGeneralDocumentation(List<Map<String, dynamic>> ops) {
    ...
    ops.add({
      "insert":
          "Regular updates should be applied during the scheduled maintenance window (Sundays 02:00 UTC).\n",
    });
  ...
  void _addAppendices(List<Map<String, dynamic>> ops) {
    ...
    ops.add({
      "insert": "• CI/CD: Continuous Integration / Continuous Deployment\n",
    });
    ...
    ops.add({
      "insert":
          "v1.0 - Initial Draft - ${DateTime.now().subtract(Duration(days: 10)).toString().split(' ')[0]}\n",
    });
    ops.add({
      "insert":
          "v1.1 - Added Security Section - ${DateTime.now().subtract(Duration(days: 2)).toString().split(' ')[0]}\n",
    });
  }
}
...
void main(List<String> args) async {
  ...
  try {
    ...
    for (var app in apps) {
      ...
      final numDocs = 4;
      print(
        'Seeding $numDocs detailed documents for project: ${app.appName}...',
      );
