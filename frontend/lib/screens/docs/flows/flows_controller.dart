...
class FlowsController extends GetxController {
  ...
  final RxList<rpc.FlowItem> savedFlows = <rpc.FlowItem>[].obs;
  ...
  final Rx<FlowSubPage> currentSubPage = FlowSubPage.docs.obs;
  ...
  void selectApp(String appId) {
    ...
    // Update global appId
    appId = appId;
    // Load flows and docs data when app is selected
    loadSavedFlows();
    loadSavedDocs();
    // Switch to flows mode
    sidebarMode.value = SidebarMode.flows;
  }

  // Method to go back to app selection
  void backToApps() {
    selectedAppId.value = '';
    sidebarMode.value = SidebarMode.apps;
    // Clear flows and docs
    savedFlows.clear();
    flows.clear();
    selectedFlowId.value = '';
    // Clear docs via DocumentEditorProvider
    final docProvider = Get.find<DocumentEditorProvider>();
    docProvider.savedDocs.clear();
    docProvider.selectedDocId.value = '';
  }
  ...
  Future<void> loadSavedFlows() async {
    try {
      ...
      final response = await server.docs.flows.getFlows(
        rpc.ListFlowsParams(appId: selectedAppId.value),
      );
  ...
  Future<void> loadSavedDocs() async {
    final docProvider = Get.find<DocumentEditorProvider>();
    await docProvider.loadSavedDocs(selectedAppId.value);
  }
  ...
  Future<void> loadFlow(String flowId) async {
    try {
      ...
      final response = await server.docs.flows.getFlow(
        rpc.GetFlowParams(flowId: flowId),
      );
  ...
  void selectDoc(String docId) {
    // Update selected doc via DocumentEditorProvider
    final docProvider = Get.find<DocumentEditorProvider>();
    final doc = docProvider.savedDocs.firstWhere((d) => d.id == docId);
    docProvider.loadDoc(doc);
    // Clear selected flow when selecting a doc
    selectedFlowId.value = "";
  }

  // Method to clear selected doc
  void clearSelectedDoc() {
    // Clear selected doc via DocumentEditorProvider
    final docProvider = Get.find<DocumentEditorProvider>();
    docProvider.clearSelectedDoc();
    refresh();
  }
  ...
  void _repositionFlowTree(int parentId, double parentX) {
    ...
    for (var flow in flows) {
      if (flow.pid == parentId && flow.direction == Direction.down) {
        flow.y =
            flows[parentId].y +
            flows[parentId].height +
            flows[parentId].down.lineHeight;
    ...
    for (var flow in flows) {
      if (flow.pid == parentId && flow.direction == Direction.left) {
        flow.y =
            flows[parentId].y + flows[parentId].height / 2 - flow.height / 2;
        flow.x =
            flows[parentId].x - flows[parentId].left.lineHeight - flow.width;
        _repositionFlowTree(flow.id, flow.x + flow.width / 2);
      }
    }
    ...
    for (var flow in flows) {
      if (flow.pid == parentId && flow.direction == Direction.right) {
        ...
        flow.x =
            flows[parentId].x +
            flows[parentId].right.lineHeight +
            flows[parentId].width;
        _repositionFlowTree(flow.id, flow.x + flow.width / 2);
      }
    }
  }
  ...
  Future<void> save() async {
    try {
      ...
      if (flowId.value.isEmpty) {
        print(
          'No flow ID available for update. Use createFlow() to create a new flow.',
        );
        return;
      }
      ...
      final response = await server.docs.flows.updateFlow(
        rpc.UpdateFlowParams(flowId: flowId.value, flow: flowData),
      );
  ...
  Future<void> createFlow(String flowName) async {
    try {
      ...
      final response = await server.docs.flows.addFlow(
        rpc.CreateFlowParams(
  ...
  void fromJson(List flowsData) {
    ...
    for (var flowData in flowsData) {
      ...
      FlowClass flow = FlowClass(
        id: flowData["id"],
        width: (flowData["width"] as num).toDouble(),
        height: (flowData["height"] as num).toDouble(),
        x: (flowData["x"] as num).toDouble(),
        y: (flowData["y"] as num).toDouble(),
        type: type,
        value: flowData["value"],
        down: Line(
          lineHeight: (flowData["down"]["lineHeight"] as num).toDouble(),
          hasChild: flowData["down"]["hasChild"],
        ),
        left: Line(
          lineHeight: (flowData["left"]["lineHeight"] as num).toDouble(),
          hasChild: flowData["left"]["hasChild"],
        ),
        right: Line(
          lineHeight: (flowData["right"]["lineHeight"] as num).toDouble(),
